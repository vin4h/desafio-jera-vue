...
export default {
  ...
  methods: {
    ...mapActions('dashboard', ['findPageMovieRecommended', 'createWatched', 'createToWatch', 'findMoviesRecommended']),
    async watched (movie) {
      try {
    ...
    async nextPageMovie () {
      try {
        await this.findPageMovieRecommended({ page: this.movies.page + 1, genre_id: this.genre_id })
      } catch (error) {
    ...
    async search () {
      if (this.searchField.length > 0) {
        await this.findMoviesRecommended({ title: this.searchField, genre_id: this.genre_id })
      } else {
        await this.findPageMovieRecommended({ page: 1, genre_id: this.genre_id })
      }
    }
  },
  computed: {
    ...mapState('dashboard', ['movies', 'profile_id', 'genre_id']),
    rows () {
